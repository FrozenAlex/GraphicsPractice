...
{
    /// <summary>
    /// Логика взаимодействия для Lab2Task1.xaml
    /// </summary>
    public partial class Lab2Task1 : Window
    {
        ...
        public void CalculatePoints()
        {
            ...
            {
                line.end = new PointF(
                    line.end.X* MathF.Cos(angle)-line.end.Y*MathF.Sin(angle),
                    line.end.X * MathF.Sin(angle) + line.end.Y * MathF.Cos(angle)
                );
                line.start = new PointF(
                    line.start.X * MathF.Cos(angle) - line.start.Y * MathF.Sin(angle),
                    line.start.X * MathF.Sin(angle) + line.start.Y * MathF.Cos(angle)
                );
            });
        }
